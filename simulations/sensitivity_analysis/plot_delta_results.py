...
method = 'ipssrf'
parameter = 'delta'
deltas_to_plot = None #np.round(np.linspace(0.5, 1.2, 8),1)
...
for i, p in enumerate(p_grid):
	...
	if deltas_to_plot is None:
		reset = True
		deltas_to_plot = delta_grid
		color_dict = None
	else:
		color_dict = dict(zip(deltas_to_plot, colors))
		...
		for cutoff in cutoff_grid:
			for delta in delta_grid:
				if delta in deltas_to_plot:
					...
					if color_dict:
						ax[0,i].plot(fdr_list, tpr_means, lw=lw, color=color_dict[delta], label=f'$\\delta = {delta}$')
						ax[1,i].plot(fdr_list, fdr_means, lw=lw, color=color_dict[delta], label=f'$\\delta = {delta}$')
					else:
						ax[0,i].plot(fdr_list, tpr_means, lw=lw, label=f'$\\delta = {delta}$')
						ax[1,i].plot(fdr_list, fdr_means, lw=lw, label=f'$\\delta = {delta}$')
...
ax[0,len(p_grid)-1].legend(handles=custom_handles, labels=labels, loc='best', fontsize=12)

plt.tight_layout()
if save_figure:
	plt.savefig(f'sensitivity_delta_{response_type}.png', dpi=300)
plt.show()