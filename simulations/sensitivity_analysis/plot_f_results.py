# Analyze dependence on function
...
method = 'ipssgb'
parameter = 'function'
functions_to_plot = ['h1', 'h2', 'h3']

# plot args
color = {'h1':'red', 'h2':'orange', 'h3':'dodgerblue'}
name = {'h1':f'h_1', 'h2':f'h_2', 'h3':f'h_3'}
lw = 3
fontsize = 32

fig, ax = plt.subplots(2, len(p_grid), sharey=False, figsize=(18, 8))
	...
	for function in function_grid:
		for cutoff in cutoff_grid:
			for delta in delta_grid:
				if function in functions_to_plot:
					tpr_means = np.mean(tpr_matrix[function][cutoff][delta], axis=0)
					fdr_means = np.mean(fdr_matrix[function][cutoff][delta], axis=0)
					ax[0,i].plot(fdr_list, tpr_means, lw=lw, color=color[function], label=f'${name[function]}$')
					ax[1,i].plot(fdr_list, fdr_means, lw=lw, color=color[function], label=f'${name[function]}$')
...
plt.tight_layout()
if save_figure:
	plt.savefig(f'sensitivity_f_{response_type}.png', dpi=300)
plt.show()
