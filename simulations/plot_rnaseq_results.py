...
response_type = 'reg'
################################################################
figure_name = f'{experiment}_{response_type}'
################################################################

#--------------------------------
# Simulations to plot
#--------------------------------
p_to_plot = [500, 2000, 5000]
...
method_specs = {
	...
	'deeppink':{'name':'DeepPINK', 'color':'deeppink', 'linestyle':'--', 'lw':lw_nonparam_methods_with_fdr}, # Preselect 100 with RF preselection
	...
	'koglm':{'name':'KOGLM', 'color':'deepskyblue', 'linestyle':'--', 'lw':lw_param_methods_with_fdr}, # Preselect 200 with RF preselection
	'koglm_preGB1.5':{'name':'KOGLM(GB1.5)', 'color':'deepskyblue', 'linestyle':':', 'lw':lw_param_methods_with_fdr},
	'koglm_preRF100':{'name':'KOGLM(RF100)', 'color':'deepskyblue', 'linestyle':'-.', 'lw':lw_param_methods_with_fdr},
	'koglm_preRF500':{'name':'KOGLM(RF500)', 'color':'deepskyblue', 'linestyle':'-', 'lw':lw_param_methods_with_fdr},
	'kol':{'name':'KOL1', 'color':'orange', 'linestyle':'--', 'lw':lw_param_methods_with_fdr}, # Preselect 200 with L1 preselection
	'kol_preL100':{'name':'KOL(L100)', 'color':'orange', 'linestyle':'-.', 'lw':lw_param_methods_with_fdr},
	'kol_preL300':{'name':'KOL(L300)', 'color':'orange', 'linestyle':':', 'lw':lw_param_methods_with_fdr},
	'korf':{'name':'KORF', 'color':'limegreen', 'linestyle':'--', 'lw':lw_nonparam_methods_with_fdr}, # Preselect 200 with RF preselection
	'korf_preRF150':{'name':'KORF(RF150)', 'color':'limegreen', 'linestyle':'-.', 'lw':lw_param_methods_with_fdr},
	'korf_preRF500':{'name':'KORF(RF500)', 'color':'limegreen', 'linestyle':':', 'lw':lw_param_methods_with_fdr},
...
gs = GridSpec(2, len(p_to_plot) + 1, width_ratios=[1]*len(p_to_plot) + [0.2])
ax = np.array([[fig.add_subplot(gs[r, c]) for c in range(len(p_to_plot))] for r in range(2)])

for i, p in enumerate(p_to_plot):

	ax[0,i].set_title(f'$\\mathbf{{p = {p}}}$', fontsize=fontsize)

	print(f'\nRNA ({response_type}, p = {p})')
	print(f'--------------------------------')
...
target_fdr_location = 1 if len(p_to_plot) == 3 else 0
